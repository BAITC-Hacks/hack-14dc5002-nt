...
Общая точка интеграции: **main**; рабочая ветка участника 3: **Bakytzhan**.
Основные этапы: `d9813f6` — движок и расчёт,
`c128a48` — события и клиент, `ad2f72d` — AI/языки/защита/HTTP-проверки.
Брать актуальную вершину ветки вместе с последним отчётом, а не только один коммит:
этапы зависят друг от друга. Объединённая версия включает интерфейс Semba `c0bf5d6`.
Сембе нужно в своей ветке выполнить `git fetch origin`, затем `git merge origin/main`
после сохранения незакоммиченных изменений. Чужие рабочие ветки не перезаписываются.

Сообщение для передачи вручную:

> Сервер включён в общую версию main: organizer-v1, реальный simulate, отмена и обязательная мера,
> preview/confirm, explain RU/KK/EN. Используй src/lib/organizer-api.ts и
> src/contracts/organizer-api.ts. Нужно добавить районы для district-мер,
> перейти на selections и новые показатели, различать два вида событий,
> передавать addedSelection.districtId и отображать observations/source объяснения.
> API-ключ тебе не нужен: шаблоны и mock-fixtures работают без него.
> Пошаговые примеры и результаты проверок — docs/ORGANIZER-INTEGRATION.md и docs/DEMO.md.
